...
class haffman_encode {
  ...
  void haffmanencode([String? target_path]) {
    ...
    //创建写入流
    //IOSink write = file_write.openWrite();
    file_write.createSync(recursive: true);
    ...
    while (fileposition < filelength) {
      ...
      if ((read_bufferPosition >= read_buffer.length) ||
        ...
        if (read_bufferPosition >= read_buffer.length) {
          ...
          int readbufferSize =
              read_file.readIntoSync(read_buffer, 0, read_buffer.length);
          ...
          int readbufferSize =
              read_file.readIntoSync(read_buffer, 0, read_bufferPosition);
          for (int num = 0; num < readbufferSize; num++) {
            String hufCode = list2[read_buffer[num]];
            for (int j = 0; j < hufCode.length; j++) {
              if (hufCode[j] == '0') {
...
/*void main()
{
  var haffman=haffman_encode('C:\\Users\\fander\\Desktop\\yande.re 507843 cleavage dress hashima_izumi hyou (2).png');
  haffman.haffmanencode('C:\\Users\\fander\\Desktop\\新建文件夹');
} */
